...
def getFBUserid(token, user_id):
    ...
    if r.status_code != requests.codes.ok:
        print r.text
        return
    user = json.loads(r.content) # Convert Python dictionary to JSON object.
    return user

def showTyping(token, user_id, action='typing_on'):
...
def sendBackMsg(token, user_id, text):
    """Send the message text to recipient with id recipient.
    """
    r = requests.post("https://graph.facebook.com/v2.6/me/messages",
                      params={"access_token": token},
                      data=json.dumps({ # Convert JSON object to Python Dictionary data structure.
                          "recipient": {"id": user_id},
                          "message": {"text": text.decode('unicode_escape')}
                      }),
                      headers={'Content-type': 'application/json'}) # This is the MIME Type: This tells
                                                                    # the browser that the python script 
                                                                    # returns the data in the form of JSON Object.
    if r.status_code != requests.codes.ok:
        print r.text

def sendBackPicture(token, user_id, imageUrl, title="", subtitle=""):
    if title != "":
...
def sendQuickReplies(token, user_id):
    # An example of structure os options and Object.
    ...
    data = data.encode('utf-8') # encode the data in UTF-8 format for further enhanced support.
    r = requests.post("https://graph.facebook.com/v2.6/me/messages",
        params={"access_token": token},
        data=data,
        headers={'Content-type':'application/json'})
...
def sendQuickRepliesSaveLocation(token, user_id, location=None):
    # options = [Object {name:value, url:value}, Object {name:value, url:value}]
    quickRepliesOptions = [
        {"content_type":"text",
         "title": "Sure",
...
def sendQuickRepliesSuggestLocation(token, user_id, locations):
    quickRepliesOptions = []
    i = 0
...
def sendBackResults(token, user_id, businesses):
    options = []
...
def sendBackTrendingNews(token, user_id, posts):
    options = []
    for post in posts:
